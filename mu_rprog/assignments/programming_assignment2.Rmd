...
* `get_dataset()`
    - takes 2 arguments:
        ...
        - `num_rows`: an integer. If `> -`, the function returns the first `n` rows of the data. If `<= 0`, the function returns all rows.
...
Follow the template below, only adding code code between the `###` comments.
...
# throw and error if returns value does not have the expected number of rows and columns
