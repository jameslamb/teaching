...
For the remainder of this assignment, I'll refer to **the package list**. Please see [the list on GitHub](https://github.com/jameslamb/teaching/blob/master/mu_rprog/docs/assignments/final_project_packages.md).
...
Using the resources I've provided below, your goal is to download two series: Japanese inflation (YOY percent change) and U.S. inflation (YOY percent change), join them into a single data.frame, and plot them against each other on a line plot using R's base plotting system.
...
Upload a file to the **Programming Assignment 2** dropbox on D2L with a name like *firstname_lastname_assignment2pt2.R*. In the example script below, I've filled in the pieces needed to pull the U.S. data and plot it. Your task is to fill out the rest of this script so that Japan is added to the same plot.
...
- add the Japanese data to the plot with `lines(type = "l")`
...
mergedDF <- merge(x = usaDF, by = "Date", y = ### FILL THIS IN ###)
                      
# Plot the two series
plot(x = mergedDF$Date, 
     y = mergedDF$USA_INF_YOY,
     type = ### FILL THIS IN ###,
     main = ### FILL THIS IN ###,
     xlab = ### FILL THIS IN ###,
     ylab = ### FILL THIS IN ###)
         
lines(x = ### FILL THIS IN ###, type = "l", col = ### FILL THIS IN ###)
```
