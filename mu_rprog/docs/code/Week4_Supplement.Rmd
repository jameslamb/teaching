...
In this exercise, we're going to evaluate the following question: **Can we prodict fertility based on regional socioeconomic characteristics?**
...
Ok now that we've done some basic preprocessing and exploration, it's time to start fitting models! Let's begin with a simple one-variable OLS regress, estimated using the `lm` command.
...
The error metric calculations and plotting we just did seem general enough to apply to other models of this phenomenon. Since we could reuse the code for other models, we should just wrap it in a funciton so it's easy to call downstream.
...
createSentence <- function(){
    ...
    # Define a list of verbs
    verbs <- c("scurried", "trunled", "rolled", "walked")
